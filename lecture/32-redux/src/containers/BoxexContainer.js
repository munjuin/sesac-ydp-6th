...
export const Box4Container =()=>{
  ...
  return <Box4 number={number} onPluse={()=> dispatch(plus())} onMinus={()=> dispatch(minus())} />
}

// 컨테이너 컴포넌트 & 프레젠테이셔널 컴포넌트
// 리액트 애플리케이션에서 컴포넌트를 구성하고 관리하는 두 가지 주요 패턴이 있음
...
// 주로 입력(props)만 받고 출력(UI)을 렌더링 함
